...
/*   Updated: 2023/10/24 15:46:52 by tpoho            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class Join
{
	public:
		...
		static void joinCommand(const int		&socket,
	...
	private:
		...
		static int	_handleSpecialCases(const int						&socket,
										const std::vector<std::string>	&command_parts,
										t_server_mode					&_serverSettings);

		static void _channelDoesNotExistHelper(	const int 									&socket,
		...
		static void	_inviteOnlyErrorHelper(	const int								&socket,
											const std::vector<Channel>::size_type	&k,
											t_server_mode							&_serverSettings);

		static void _keyMatchesHelper(	const int								&socket,
										const std::string						&full_command,
										const std::vector<Channel>::size_type	&k,
										t_server_mode							&_serverSettings);

		static void _keyDoesNotMatchHelper(	const int								&socket,
											const std::vector<Channel>::size_type	&k,
											t_server_mode							&_serverSettings);
		...
		static void _channelDoesNotHaveKeyHelper(	const int								&socket,
													const std::vector<Channel>::size_type	&k,
													const std::string						&full_command,
													t_server_mode							&_serverSettings);
};
